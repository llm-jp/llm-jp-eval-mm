...
def plot_correlation(df: pd.DataFrame, filename: str, tex_columns: list[str] = None):
    ...
    # Title and colorbar in Japanese
    ax.set_title('タスク間の相関行列（スピアマン）', fontsize=22, fontweight='bold')
    if hm and hasattr(hm, 'collections') and hm.collections:
        cbar = hm.collections[0].colorbar
        if cbar:
            cbar.set_label('スピアマン相関', fontsize=16)
            # also enlarge cbar tick labels
    ...
    if 0 < en_count < len(corr.columns):
        ...
        ax.axhline(boundary, color='black', linestyle=':', linewidth=1)
...
def plot_task_clustering(df: pd.DataFrame, filename: str, tex_columns: list[str] = None):
    ...
    plt.title('相関行列に基づくタスク階層クラスタリング', fontsize=28, fontweight='bold')
    plt.xlabel('距離（1 - スピアマン相関）', fontsize=20, fontweight='bold')
    plt.ylabel('タスク', fontsize=20, fontweight='bold')

    # Make y-axis labels bold and color Japanese tasks red
    ax = plt.gca()
